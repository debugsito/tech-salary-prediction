...
Sustituye a `feature_pipeline.py`, construido para el esquema del conjunto de
datos sintético descartado y cuyas columnas no existen en la encuesta real.
...
from src.encoders import MeanTargetEncoder
...
def construir_preprocesador(df: pd.DataFrame, codificacion: str = "target",
    ...
    if codificacion == "onehot":
        ...
        t_alta = Pipeline([
            # MeanTargetEncoder espera códigos numéricos, no etiquetas de texto.
            ("ordinal", OrdinalEncoder(handle_unknown="use_encoded_value",
                                       unknown_value=-1)),
            ("objetivo", MeanTargetEncoder(smoothing=suavizado)),
        ])
    else:
