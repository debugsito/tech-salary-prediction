...
No ejecuta nada: lee los artefactos ya producidos por `run_experiment.py`
(la edición 2023 en `results/`, las otras en `results/ediciones/<año>/`),
los resúmenes SHAP de `run_analysis.py` y las auditorías de equidad.

Artefactos: results/consistencia_ediciones.json

Uso:
    python scripts/run_consistencia.py
"""
...
def resumen_edicion(anio: str, ruta: Path) -> dict:
    cv = pd.read_csv(ruta / "cv_by_encoding.csv")
    meta = json.loads((ruta / "run_metadata.json").read_text(encoding="utf-8"))
    ...
    wil = pd.read_csv(ruta / "wilcoxon_tests.csv")
    ...
    # Jerarquía SHAP por bloque, del resumen que run_analysis exporta.
    shap = RES / f"shap_summary_XGBoost_target_{anio}.csv"
    if shap.exists():
    ...
    fair = RES / f"fairness_wb_region_{anio}.json"
    if fair.exists():
