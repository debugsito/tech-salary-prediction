...
Una matriz sí se contrasta. Este script toma de `src/feature_pipeline_so.py` qué
...
Uso:
    python scripts/generate_matriz_operacionalizacion.py
"""
...
from src.feature_pipeline_so import (CAT_ALTA, CAT_BAJA, NUMERICAS,  # noqa: E402
                                     OBJETIVO, columnas_tecnologia)

DESTINO = RAIZ.parent / "tesis_borrador" / "06_capitulo_3_metodologia.md"
...
def construir(df: pd.DataFrame) -> list[str]:
    ...
    l += ["", "*Fuente:* generada desde `src/feature_pipeline_so.py` y la muestra "
              "efectiva de la edición 2023 mediante "
              "`scripts/generate_matriz_operacionalizacion.py`.", ""]
    return l
...
def main() -> int:
    ...
    if MARCA_INICIO not in texto:
        print(f"No se encuentran las marcas en {DESTINO.name}.")
        print("Insertar donde deba ir la matriz:")
        print(f"  {MARCA_INICIO}\n  {MARCA_FIN}")
        return 1
