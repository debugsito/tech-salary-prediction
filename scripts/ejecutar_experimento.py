...
Artefactos producidos en `resultados/`:

    cv_por_codificacion.csv     Métricas por modelo, estrategia y partición
    prueba_por_codificacion.csv   Métricas sobre el conjunto de prueba
    contrastes_wilcoxon.csv     Contraste de significancia entre configuraciones
    metadatos_ejecucion.json      Semilla, versiones, tiempos y tamaños de partición

Uso:
    python scripts/ejecutar_experimento.py                    # ejecución completa
    python scripts/ejecutar_experimento.py --muestra 4000     # validación rápida
    python scripts/ejecutar_experimento.py --anio 2022
"""
...
def construir_configuraciones() -> dict:
    ...
    modelos.update(construir_modelos())
    return modelos
...
def main():
    ...
    for codificacion in args.codificaciones:
        ...
        for nombre, modelo in modelos.items():
            ...
            try:
                ...
                continue
            m = calcular_metricas(y_prueba, pred, prefijo="test_")
            m.update({"modelo": nombre, "codificacion": codificacion})
            test_todas.append(m)
        print()
    ...
    test.to_csv(salida / "prueba_por_codificacion.csv", index=False)
    ...
    pruebas.to_csv(salida / "contrastes_wilcoxon.csv", index=False)
    ...
    (salida / "metadatos_ejecucion.json").write_text(
