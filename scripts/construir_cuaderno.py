...
Uso:
    python scripts/construir_cuaderno.py
...
# resultados/ y figuras/, que son los artefactos del proyecto. Si una celda falla a
# mitad, prefiero que no se lleve por delante nada que ya estaba bien.
SALIDA = RAIZ / "salida_cuaderno" / "resultados"
FIGURAS = RAIZ / "salida_cuaderno" / "figuras"
...
"""),
(CODE, r"""
from src.carga_encuesta import descargar, COLUMNAS, TARGET
...
"""),
(CODE, r"""
from src.carga_encuesta import normalizar_ausentes
...
from src.carga_encuesta import derivar_region, cargar_referencia_paises
...
"""),
(CODE, r"""
from src.carga_encuesta import parsear_anios, TRAMO_EDAD_MAX
...
"""),
(CODE, r"""
from src.carga_encuesta import expandir_multivalor, CATEGORIA_AUSENTE
...
"""),
(CODE, r"""
from scripts.generar_figuras import estilo, fig_distribucion, fig_composicion
estilo()
...
"""),
(CODE, r"""
from src.tuberia_caracteristicas import preparar_xy, CAT_ALTA, CAT_BAJA, NUMERICAS
...
"""),
(CODE, r"""
from src.tuberia_caracteristicas import construir_preprocesador, nombres_de_variables
...
from src.registro_modelos import construir_modelos
    ...
    for i, (tr, va) in enumerate(particionador.split(X_ent, estratos)):
        for codificacion in ("target", "onehot"):
            for nombre, modelo in modelos.items():
                ...
                m = calcular_metricas(y_ent[va], t.predict(X_ent.iloc[va]))
...
prueba.to_csv(SALIDA / "prueba_por_codificacion.csv", index=False)
...
"""),
(CODE, r"""
from scripts.generar_figuras import fig_residuos
...
cv = correr_cv(4)
cv.to_csv(SALIDA / "cv_por_codificacion.csv", index=False)
pivote = cv.pivot_table(index="particion", columns=["modelo", "codificacion"], values="R2")
...
from scripts.generar_figuras import fig_modelos
...
contrastes = pd.concat([holm(b) for _, b in pd.DataFrame(comparaciones).groupby("familia")],
                       ignore_index=True)
contrastes.to_csv(SALIDA / "contrastes_wilcoxon.csv", index=False)
...
"""),
(CODE, r"""
from scripts.generar_figuras import fig_shap
...
"""),
(CODE, r"""
imp = pd.read_csv(SALIDA / f"shap_resumen_{mejor[0]}_{mejor[1]}_2023.csv")
...
"""),
(CODE, r"""
from ejecutar_interacciones import fuerza_interaccion
...
"""),
(CODE, r"""
from scripts.generar_figuras import fig_interaccion_experiencia
...
p22 = t22.predict(Xp)
print(f"\nR² sobre prueba: {calcular_metricas(yp, p22)['R2']:.4f}")

aud_g = auditar(yp, p22, Ap["Gender"])
(SALIDA / "equidad_Gender_2022.json").write_text(
...
"""),
(CODE, r"""
from ejecutar_mitigacion import pesos_inversos, remuestrear
...
from scripts.generar_figuras import fig_mitigacion
...
from ejecutar_analisis import analizar_ia
...
imp25 = pd.read_csv(SALIDA / f"shap_resumen_{mejor[0]}_{mejor[1]}_2025.csv")
ia_vars = imp25[imp25["variable"].str.startswith(("AISelect", "AIThreat", "AIAgents", "LearnCodeAI"))]
print(f"R² sobre prueba (2025): {calcular_metricas(yp25, t25.predict(Xp25))['R2']:.4f}\n")
...
"""),
(CODE, r"""
from scripts.generar_figuras import fig_ia
...
"""),
(CODE, r"""
ppp = pd.read_csv(RAIZ / "datos/referencia/factores_ppp.csv")
...
def entrenar_y_medir(objetivo_log, etiqueta):
    ...
    r2 = calcular_metricas(yr_, t.predict(Xr_))["R2"]
...
oews = json_lib.loads((RAIZ / "datos/referencia/oews_medianas_2025.json").read_text())
...
ruta_aj = RAIZ / "datos/descargas/aijobs_salaries.csv"
...
m_ext = calcular_metricas(np.log1p(d.salario.to_numpy()), tuberia.predict(X_ext))
...
m_masc = calcular_metricas(y_pru, tuberia.predict(X_masc))
m_full = calcular_metricas(y_pru, tuberia.predict(X_pru))
...
`scripts/ejecutar_consistencia.py`; aquí me basta el corte rápido.
"""),
(CODE, r"""
consist = []
for anio, dfe in [("2022", df22), ("2023", df), ("2025", df25)]:
    ...
    consist.append({"edición": anio, "n": len(dfe),
                    "R²": round(calcular_metricas(ypr, pr_)["R2"], 3),
