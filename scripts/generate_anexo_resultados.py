...
Salida: `tesis_borrador/12_anexo_resultados.md`

Uso:
    python scripts/generate_anexo_resultados.py
"""
...
RES = RAIZ / "results"
SALIDA = RAIZ.parent / "tesis_borrador" / "12_anexo_resultados.md"
...
def seccion_shap(l: list[str]):
    p = RES / "shap_summary_XGBoost_target_2023.csv"
    if not p.exists():
        return
...
def seccion_equidad(l: list[str]):
    ...
    for atributo, anio, titulo in [
        ...
        p = RES / f"fairness_{atributo}_{anio}.json"
        if not p.exists():
            continue
    ...
    l += ["*Fuente:* `results/fairness_*.json`", ""]
...
def main():
    l = ["# Anexo I: Tablas de resultados completas", "",
         ...
         "mediante `scripts/generate_anexo_resultados.py`. Ninguna cifra se "
         "transcribe a mano.", "", "---", ""]
