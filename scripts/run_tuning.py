...
Salida:
    results/tuning_resultados.csv   Configuración óptima y desempeño por modelo
    results/tuning_espacio.csv      Todas las combinaciones evaluadas

Uso:
    python scripts/run_tuning.py                    # búsqueda completa
    python scripts/run_tuning.py --iteraciones 20   # búsqueda más breve
"""
...
def main():
    ...
    pd.DataFrame(filas).to_csv(salida / "tuning_resultados.csv", index=False)
    pd.DataFrame(espacio).to_csv(salida / "tuning_espacio.csv", index=False)
