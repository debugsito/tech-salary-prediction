...
Artefactos: results/transferencia_aijobs.json

Uso:
    python scripts/run_transferencia.py
"""
...
from src.feature_pipeline_so import preparar_xy  # noqa: E402
from src.metrics import compute_metrics  # noqa: E402

SEMILLA = 42
CSV = RAIZ / "data" / "external" / "aijobs_salaries.csv"
MODELO = RAIZ / "servicio" / "artefactos" / "modelo.joblib"
CONTEXTO = RAIZ / "servicio" / "artefactos" / "contexto.json"
SALIDA = RAIZ / "results" / "transferencia_aijobs.json"
...
def main() -> int:
    ...
    for nombre, ancla in ANCLAS.items():
        ...
        m = compute_metrics(np.log1p(d.salario.to_numpy()), pred)
    ...
    completa = compute_metrics(y_pru, tuberia.predict(X_pru))
    ...
    enmascarada = compute_metrics(y_pru, tuberia.predict(X_masc))
    ...
    solo_usa = compute_metrics(np.log1p(real_usd[sel_usa]),
