...
Produce `resultados/informe_muestra.json` y `resultados/informe_muestra.md` con el efecto
...
Uso:
    python scripts/generar_informe_muestra.py
"""
...
def caracterizar(df: pd.DataFrame, registro: pd.DataFrame) -> dict:
    ...
    if "LanguageHaveWorkedWith" in df.columns:
        ...
        from src.carga_encuesta import expandir_multivalor
...
def a_markdown(todo: dict) -> str:
    l = ["# Cifras de muestra", "",
         "> Generado por `scripts/generar_informe_muestra.py`. **No editar a mano.**",
...
def main():
    ...
    (out / "informe_muestra.json").write_text(
        json.dumps(todo, indent=2, ensure_ascii=False), encoding="utf-8")
    (out / "informe_muestra.md").write_text(a_markdown(todo), encoding="utf-8")
    print(f"\nEscritos {out}/informe_muestra.json y {out}/informe_muestra.md")
