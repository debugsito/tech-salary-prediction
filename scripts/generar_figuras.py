...
Todas las figuras se producen desde los archivos de `resultados/` y `data/`, nunca
desde cifras transcritas a mano, conforme al criterio de trazabilidad de §3.6.1.

Salida: `figuras/*.png` a 200 puntos por pulgada, tamaño ajustado al ancho de
página del documento.

Uso:
    python scripts/generar_figuras.py
    python scripts/generar_figuras.py --solo dist_compensacion
"""
...
def fig_modelos(salida, resultados: Path):
    ...
    cv = pd.read_csv(resultados / "cv_por_codificacion.csv")
...
def fig_shap(salida, resultados: Path, etiqueta="XGBoost_target_2023", n=15):
    ...
    ruta = resultados / f"shap_resumen_{etiqueta}.csv"
    if not ruta.exists():
        return None
...
def fig_equidad(salida, resultados: Path, anio="2023"):
    ...
    ruta = resultados / f"equidad_wb_region_{anio}.json"
    if not ruta.exists():
        return None
...
def fig_ia(salida, resultados: Path):
    ...
    s = resultados / "shap_resumen_XGBoost_target_2025.csv"
    if s.exists():
        sh = pd.read_csv(s)
...
def fig_residuos(salida, df, resultados: Path):
    ...
    from src.tuberia_caracteristicas import construir_preprocesador, preparar_xy
...
def main():
    ...
    ap.add_argument("--out-dir", default="figuras")
    ap.add_argument("--results-dir", default="resultados")
    args = ap.parse_args()
