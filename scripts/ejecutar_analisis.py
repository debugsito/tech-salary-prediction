...
de prueba y produce los artefactos que sustentan el Capítulo V:
    ...
    shap_resumen_<modelo>.csv   Importancia media por variable, con intervalos
    shap_values_<modelo>.npz    Valores calculados, para verificación independiente
    equidad_<atributo>.json    Auditoría de equidad por atributo
    informe_equidad.txt         Informe legible de todas las auditorías
    ia_asociacion.csv           Asociación entre adopción de IA y compensación (2025)

Uso:
    python scripts/ejecutar_analisis.py                       # edición 2023, XGBoost
    python scripts/ejecutar_analisis.py --anio 2022           # incluye género
    python scripts/ejecutar_analisis.py --anio 2025 --ia      # análisis de adopción de IA
"""
...
from src.explicabilidad import explicar  # noqa: E402
from src.equidad import auditar, resumir  # noqa: E402
...
def main():
    ...
    metricas = calcular_metricas(y_pru, pred, prefijo="test_")
    ...
    for atributo in ATRIBUTOS_AUDITORIA:
        ...
        print()
        print(informe)
        (salida / f"equidad_{atributo}_{args.anio}.json").write_text(
            json.dumps(aud, indent=2, ensure_ascii=False), encoding="utf-8")

    (salida / f"informe_equidad_{args.anio}.txt").write_text(
