...
Uso:
    python scripts/exportar_modelo_servicio.py
"""
...
from src.tuberia_caracteristicas import (columnas_tecnologia,  # noqa: E402
...
def nivel_precios() -> dict[str, float]:
    ...
    import csv
    ruta = RAIZ / "datos" / "referencia" / "factores_ppp.csv"
...
def main() -> int:
    ...
    tuberia = Pipeline([
        ...
        ("modelo", construir_modelos()[MODELO]),
    ]).fit(X_ent, y_ent)

    metricas = calcular_metricas(y_pru, tuberia.predict(X_pru))
    ...
    auditoria = json.loads((RAIZ / "resultados" / "equidad_income_group_2023.json")
