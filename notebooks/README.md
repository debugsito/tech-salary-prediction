...
| :--- | :--- | :--- |
| `tesis_trabajo.ipynb` | `construir_cuaderno.py` | Rehacer el análisis entero desde el CSV en bruto, decisión por decisión, incluido el programa de robustez (PPA, patrón oro oficial, transferencia, consistencia entre ediciones y mitigación por modelo). Produce las nueve figuras y todas las tablas |
| `tesis_colab.ipynb` | `construir_cuaderno_colab.py` | Reejecutar el experimento y contrastar el resultado con lo que afirma el documento |
...
`tesis_trabajo.ipynb` escribe en `salida_cuaderno/`, nunca en `resultados/` ni en
`figuras/`. Es deliberado: si una celda falla a mitad no se lleva por delante los
artefactos del proyecto. Sus nueve figuras salen byte a byte idénticas a las de
`figuras/`, porque llama a las mismas funciones de dibujo.
